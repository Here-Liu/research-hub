# Consensus 使用指南：AI 文献检索与证据分析工具

## 它不只是一个论文搜索引擎

传统的 Google Scholar、PubMed 做一件事：**给你一篇篇的论文**。你搜一个关键词，出来几百篇，自己读标题、筛摘要、判断证据强弱——文献综述的前期工作可以花掉几周。

Consensus 做的事情不一样。它有两种核心用法：

**用法一：做文献调研和综述。** 像用搜索引擎一样按关键词/主题搜索，但 AI 会自动筛选高相关论文、提取方法学特征、生成摘要和对比表格。适用于文献综述前期搜索、范围界定（scoping review）和系统综述的论文筛选。

**用法二：回答具体研究问题。** 问一个有明确答案方向的问题，它在 2.2 亿+ 学术论文中检索，用 AI 对高相关论文做摘要和分类，告诉你证据指向哪个方向——Yes / No / Possibly，且关键结论附带来源论文。

两种用法共用同一数据库，只是入口和输出形态不同：

```
【用法一：文献调研】
输入：keyboard, chronic pain, ergonomics
输出：按年份/方法学/期刊排列的论文列表 + Study Snapshots + 可 Deep Search

【用法二：研究问答】
输入：Does intermittent fasting improve cognitive function?
输出：Consensus Meter（Yes 12 : Possibly 5 : No 3）+ 每篇证据摘要
```
...
|---|---|---|
| **回答基础** | 训练数据中的知识，可能过时 | 检索持续更新的 2.2 亿+ 学术论文库 |
...
Consensus 没有"文献综述模式"和"问答模式"的开关——两种用法共用同一个 2.2 亿+ 论文数据库，区别在于你输入什么、得到什么：

**做文献调研** → 用关键词/主题搜索，跟用 Google Scholar 一样，但结果页多了 AI 自动提取的方法学标签（Study Snapshots）、筛选器（19 种研究设计）、以及一键 Deep Search 生成综述草稿。你得到一个按相关度/年份/研究方法排列的论文集合。

**回答研究问题** → 用自然语言问一个问题，得到 Yes/No/Possibly 的证据分布仪表盘（Consensus Meter），外加每篇支持论文的摘要。

两种用法可以灵活切换：从文献调研中找到核心论文 → 用这些论文去问新问题 → 得到答案后再用 Citation Graph 拓展。
...
| 类型 | 例子 | 效果 |
|------|------|------|
| **文献调研 / 范围界定综述** | 按关键词/主题搜索（如 `keyboard ergonomics RSI`），用筛选器按年份/方法学分类整理 | ⭐⭐⭐⭐⭐ |
| Yes/No 研究问题 | "Does mindfulness meditation reduce anxiety in college students?" | ⭐⭐⭐⭐⭐ |
| **系统综述前期筛选** | 用 Deep Search + Study Snapshots 批量提取论文特征，快速筛出符合纳入标准的论文 | ⭐⭐⭐⭐ |
...
| 研究现状速览 | "What does the literature say about X?" | ⭐⭐⭐⭐ |
| **更新文献综述** | 导入已有 Zotero 库 → 搜索关联新论文 → Chat with Collection 对比新旧发现 | ⭐⭐⭐⭐ |
...
*见 §2.0 两种用法的区别——不想读可以快速参考：*

**做文献调研** → 直接输入关键词：`machine learning diabetes prediction`
**回答研究问题** → 输入具体问题：`Does intermittent fasting improve cognitive function?`
**找已知论文** → 用 Paper Search（底栏 Tools 下），直接搜标题/作者/DOI
...
- **Ask Paper** — 对单篇论文提问，AI 基于可获取的全文内容回答
...
（可显著缩短前期筛选和整理时间，但**不能替代人工精读和证据质量评估**）
```
...
2. **不自动区分证据等级** — Consensus 按论文计数，但计数不等于证据等级。一篇 RCT 和一篇病例报告在统计时权重相同，你需要自行判断。
...
- [Consensus Product Changelog（$30M 融资节）](https://help.consensus.app/en/articles/11954907-consensus-product-changelog#h_88365a53f7)
