...
**做文献调研** → 直接输入关键词，跟用 Google Scholar 一样：`machine learning diabetes prediction` 或 `keyboard ergonomics RSI`。结果页返回论文列表，每条附带 Study Snapshots。左侧筛选器按年份/研究设计/出版商细化。适合：文献综述前期搜索、范围界定、系统性检索。

**回答研究问题** → 输入一个具体的问题：`Does intermittent fasting improve cognitive function?`。结果页顶部显示 Consensus Meter 证据分布。适合：验证假设、找证据支撑论点、快速了解学界对一个问题的态度。

**找已知论文** → 用 Paper Search（底栏 Tools 下），直接搜标题/作者/DOI，返回论文详情无 AI 干扰。

无论哪种入口，后续都可以用 Copilot 追问、Deep Search 深挖、或加入 Collection 整理。关键词技巧：
